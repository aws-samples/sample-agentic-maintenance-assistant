...
def get_or_create_user_app_client(cognito, user_pool_id, app_client_name):
    """Create app client for user authentication (no secret, for frontend)"""
    response = cognito.list_user_pool_clients(UserPoolId=user_pool_id, MaxResults=60)
    for client in response["UserPoolClients"]:
        if client["ClientName"] == app_client_name:
            return client["ClientId"]
    
    print('Creating new user app client')
    created = cognito.create_user_pool_client(
        UserPoolId=user_pool_id,
        ClientName=app_client_name,
        GenerateSecret=False,  # No secret for frontend clients
...
def create_user_groups(cognito, user_pool_id):
    ...
    groups = [
        ...
        {
            "GroupName": "Operators", 
            "Description": "Can simulate assets and view data",
            "Precedence": 2
        },
        {
            "GroupName": "Viewers",
            "Description": "Read-only access to view assets and data",
            "Precedence": 3
        }
    ]
    ...
    for group in groups:
        try:
            ...
            print(f"Group {group['GroupName']} already exists")
        except Exception as e:
            print(f"Failed to create group {group['GroupName']}: {e}")

def create_post_confirmation_lambda(user_pool_id):
    """Create Lambda function for Cognito post-confirmation trigger"""
    import zipfile
    import os
    ...
    try:
        ...
        try:
            role_response = iam_client.create_role(
                ...
                Description="Role for Cognito post-confirmation Lambda trigger"
            )
            ...
            # Create custom policy for Cognito operations
            cognito_policy = {
                "Version": "2012-10-17",
                "Statement": [
                    {
                        ...
                        "Resource": f"arn:aws:cognito-idp:*:*:userpool/{user_pool_id}"
                    }
                ]
            }
        ...
        with zipfile.ZipFile(zip_filename, 'w') as zip_file:
            zip_file.write('cognito_post_confirmation_trigger.py', 'lambda_function.py')
        ...
        try:
            response = lambda_client.create_function(
                ...
                Description='Auto-assign new users to Administrators group',
                Timeout=30
            )
...
def create_identity_pool(user_pool_id, user_app_client_id, region):
    ...
    try:
        ...
        # Check if identity pool already exists and update it if needed
        existing_pool_id = None
        try:
            pools = cognito_identity.list_identity_pools(MaxResults=60)
            for pool in pools.get('IdentityPools', []):
                if pool['IdentityPoolName'] == 'MaintenanceAssistantIdentityPool':
                    existing_pool_id = pool['IdentityPoolId']
                    print(f"Found existing Identity Pool: {existing_pool_id}")
                    
                    # Check if it's configured for the correct User Pool
                    pool_details = cognito_identity.describe_identity_pool(IdentityPoolId=existing_pool_id)
                    providers = pool_details.get('CognitoIdentityProviders', [])
                    
                    correct_provider = f'cognito-idp.{region}.amazonaws.com/{user_pool_id}'
                    provider_matches = any(p.get('ProviderName') == correct_provider for p in providers)
                    
                    if not provider_matches:
                        print(f"Identity Pool configured for wrong User Pool, updating...")
                        # Update the existing pool with correct configuration
                        cognito_identity.update_identity_pool(
                            IdentityPoolId=existing_pool_id,
                            IdentityPoolName='MaintenanceAssistantIdentityPool',
                            AllowUnauthenticatedIdentities=False,
                            CognitoIdentityProviders=[
                                {
                                    'ProviderName': correct_provider,
                                    'ClientId': user_app_client_id,
                                    'ServerSideTokenCheck': False
                                }
                            ]
                        )
                        print(f"Updated Identity Pool with correct User Pool: {user_pool_id}")
                    else:
                        print(f"Identity Pool already configured correctly for User Pool: {user_pool_id}")
                    
                    return existing_pool_id
        except Exception as e:
            print(f"Error checking existing pools: {e}")
        
        # Create identity pool with proper provider configuration
        provider_name = f'cognito-idp.{region}.amazonaws.com/{user_pool_id}'
        print(f"Configuring Identity Pool with provider: {provider_name}")
        print(f"Client ID: {user_app_client_id}")
        ...
        # Verify the configuration by describing the identity pool
        try:
            pool_details = cognito_identity.describe_identity_pool(IdentityPoolId=identity_pool_id)
            print(f"Identity Pool providers: {pool_details.get('CognitoIdentityProviders', [])}")
        except Exception as e:
            print(f"Warning: Could not verify Identity Pool configuration: {e}")
        
        return identity_pool_id
        
    except Exception as e:
        print(f"Failed to create Identity Pool: {e}")
        return None

def create_identity_pool_roles(identity_pool_id, region):
    """Create IAM roles for different user groups with appropriate AWS permissions"""
    iam_client = boto3.client('iam')
    cognito_identity = boto3.client('cognito-identity', region_name=region)
    
    # Get user pool ID from runtime config
    import os
    ...
    for role_name, role_config in roles.items():
        try:
            ...
            for i, policy in enumerate(role_config["policies"]):
                iam_client.put_role_policy(
                    RoleName=role_name,
                    PolicyName=f"{role_name}Policy{i+1}",
    ...
    # Set up role mapping based on Cognito groups
    try:
        # Ensure we have the required roles
        if not created_roles.get("MaintenanceAdministrator"):
            raise Exception("MaintenanceAdministrator role not created")
        
        # Wait a moment for Identity Pool to be fully configured
        import time
        time.sleep(2)
        
        # Simple role mapping - use Administrator role as default for now
        admin_role_arn = created_roles.get("MaintenanceAdministrator")
        print(f"Setting authenticated role to: {admin_role_arn}")
        
        cognito_identity.set_identity_pool_roles(
            IdentityPoolId=identity_pool_id,
            Roles={
                'authenticated': admin_role_arn
            }
        )
        
        # Verify the role assignment
        try:
            roles = cognito_identity.get_identity_pool_roles(IdentityPoolId=identity_pool_id)
            print(f"Verified Identity Pool roles: {roles.get('Roles', {})}")
        except Exception as e:
            print(f"Warning: Could not verify role assignment: {e}")
        
        print("Configured Identity Pool role mappings")
        print(f"   Default authenticated role: {created_roles.get('MaintenanceViewer')}")
        print(f"   Administrator role: {created_roles.get('MaintenanceAdministrator')}")
        print(f"   Operator role: {created_roles.get('MaintenanceOperator')}")
        
    except Exception as e:
        print(f"Failed to configure role mappings: {e}")
        print(f"Available roles: {list(created_roles.keys())}")
